# -*- coding: utf-8 -*-
"""MachineLearningAssignment.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1BCgTzlQQAejv_hl8q4JGZZVyVSfOV7Un
"""
...
log_grid = {'C': [2,3,4,5,6,7], 'solver':['lbfgs','liblinear']}#'C': [4,5,6,7,8,9,10]
...
#Could do something with improvement
...
print(knn_results)
print(knn_results['probs'].unique())
print()
rf_results = pd.DataFrame({'Target':y_test,'class_result':rf_prediction_y_test,'probs':rf_probs})
rf_results['rank']=rf_results['probs'].rank(ascending=1).astype(int)
rf_results['rank_pct']=rf_results['probs'].rank(ascending=1,pct=True)
print(rf_results)
print(rf_results['probs'].unique())

bins = 10
def bin_analysis1(df,bin_num,plot):
...
def bin_analysis(df,bin_num,plot):
    ...
    for i in range(bin_num):
        if (i%10)<int((num_ins/bin_num-bin_size)*10):
            bin_maxs.append(bin_size+1)
        else:
            bin_maxs.append(bin_size)
    bin_maxs[bin_num-1]=num_ins
    df_extra=df.copy()
    df_indices = []
    for i in range(bin_num):
        df_sub = df_extra[(df_extra['rank']>=count) & (df_extra['rank']<bin_maxs[i])]
        if len(df_sub)==0:
          extra_count=bin_maxs[i]+1
          while (len(df_sub)==0 and extra_count<=num_ins):
            df_sub = df_extra[(df_extra['rank']>=count) & (df_extra['rank']<extra_count)]
            extra_count+=1
          df_sub = df_sub[:bin_maxs[i]]
          print(len(df_sub))
        df_extra.drop(list(sub_df.index.values),axis=0,inplace=True)
        count+=df_sub.shape[0]
        num_pos_instances=len(df_sub[df_sub['Target']==1])
        #print(df_sub['probs'])
        bin_df.loc[i]=([count-df_sub.shape[0],count,min(df_sub['probs']),max(df_sub['probs']),num_pos_instances,df_sub.shape[0],num_pos_instances/df_sub.shape[0]])
        df_indices.extend(list(sub_df.index.values))
        print(len(df),len(df_extra))
        sub_df_sm = df[:max(df_indices)]#df[df['rank']<bin_maxs[i]]
        sub_df_l = df_extra#df[df['rank']>=bin_maxs[i]]
        TP = len(sub_df_l[sub_df_l['Target']==1])
        FN = len(sub_df_sm[sub_df_sm['Target']==1])
        TPR = TP/(TP+FN)
        FP = len(sub_df_l.index)-TP
...
bin_analysis(rf_results,bins,True)
